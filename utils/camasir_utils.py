...
# -------------------- AYARLAR --------------------
DATA_PATH = r"C:/Users/casper/Desktop/Class_3_sprng/Müh Pro/Veri/veri_setleri/guncel_Camasır_islenmis_onehot.xlsx"
MODEL_PATH = r"C:/Users/casper/Desktop/Class_3_sprng/Müh Pro/Veri/veri_setleri/mühp2/models/camasir_model.pkl"
...
def generate_advice(status, prob, daily_kwh, threshold, df_day):
    ...
    if status == "verimsiz":
        if prob >= 0.8:
            msg = "🚨 Çamaşır makinesi çok sık çalıştırılıyor. Tam kapasiteyle çalıştırmayı ve çamaşırları biriktirerek yıkamayı düşünebilirsiniz."
        elif "is_peak" in df_day.columns and df_day["is_peak"].sum() > len(df_day) * 0.3:
            msg = "⏱️ Puant saatlerde yıkama yapılmış. Enerji tasarrufu için gece veya sabah saatleri tercih edilebilir."
        else:
            msg = "⚠️ Ortalama tüketim yüksek. Hızlı programlar veya düşük sıcaklıkta yıkama tercih edilebilir."
    else:
        if df_day["power_watt"].max() > threshold * 0.1:
            msg = "ℹ️ Gün içinde kısa süreli yüksek tüketim gözlemlendi. Sık sık kısa programlar tercih ediliyor olabilir."
        else:
            msg = "✅ Çamaşır makinesi verimli kullanılıyor. Mevcut yıkama alışkanlıklarınız uygundur."
